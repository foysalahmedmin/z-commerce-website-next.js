...
const SignInPage = async () => {
  ...
  const session = cookiesData.get("z-commerce");
  if (session?.value) {
  ...
  return (
    <>
      <section className="grid min-h-screen place-items-center py-8 xl:py-12">
        <div className="relative grid size-full grid-cols-1 grid-rows-1 place-items-center px-container-inset py-6 xl:py-8">
          ...
          <div className="absolute bottom-0 left-1/2 right-0 top-0 hidden grid-cols-1 grid-rows-1 rounded-e-md bg-background/50 pr-8 pt-8 shadow-inner md:grid">
            <Image
              className="mt-auto size-full object-contain object-right-bottom"
